...
Examples:
  mirror-local-to-remote.sh git@github.com:org/repo.git git@gitlab.com:org/repo.git
  mirror-local-to-remote.sh https://github.com/org/repo.git https://gitlab.com/org/repo.git --local-root /data/mirrors
EOF
}
...
function parse_args() {
  ...
  if [[ -z "$SOURCE_URL" || -z "$DEST_URL" ]]; then
    echo "[ERROR] source and destination URLs are required" >&2
    usage
    exit 1
  fi
}
...
function main() {
  ...
  if [[ -d "$mirror_path" ]]; then
    echo "Updating local mirror: ${mirror_path}"
    git -C "$mirror_path" remote update --prune
  else
    echo "Creating local mirror: ${mirror_path}"
    git clone --mirror "$SOURCE_URL" "$mirror_path"
  fi

  echo "Pushing mirror to destination"
  git -C "$mirror_path" push --mirror "$DEST_URL"

  echo "Mirror pushed to: ${DEST_URL}"
}
